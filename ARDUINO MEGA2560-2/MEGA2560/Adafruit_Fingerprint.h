...
uint8_t FP_storeModel(uint16_t id);
uint8_t FP_loadModel(uint16_t id);
uint8_t FP_getModel(void);
uint8_t FP_deleteModel(uint16_t id);
...
uint8_t FP_LEDcontrol(uint8_t control, uint8_t speed, uint8_t coloridx, uint8_t count);
